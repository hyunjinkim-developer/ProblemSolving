...
def sales_person(sales_person: pd.DataFrame, company: pd.DataFrame, orders: pd.DataFrame) -> pd.DataFrame:
    ...
    # Drop duplicates(in case a salesperson had multiple orders not realted to "RED")
    result = result.drop_duplicates()

    # Find the salesperson who had orders related to "RED"
    sales_with_red_orders = merged_data[merged_data["name_y"] == "RED"]["name_x"].unique()

    # Filter out sales persons with "RED" orders from the result
