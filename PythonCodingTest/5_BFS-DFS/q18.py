...
def solution(p):
    ...
    # 3_Greedy. u가 "올바른 괄호 문자열" 이라면 문자열 v에 대해 1단계부터 다시 수행
    if check_proper(u):
        answer = u + solution(v)
...
#     u, v = "".join(u), "".join(v)
#     # 3_Greedy. 문자열 u가 "올바른 괄호 문자열" 이라면 문자열 v에 대해 1단계부터 다시 수행합니다.
#     # 3_Greedy-1. 수행한 결과 문자열을 u에 이어 붙인 후 반환합니다.
